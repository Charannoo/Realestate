...
/**
 * Calls OpenRouter. Throws if OPENROUTER_API_KEY is unset.
 */
...
// POST /api/ai/chat — requires OPENROUTER_API_KEY
router.post('/chat', async (req, res) => {
    try {
        if (!OPENROUTER_API_KEY) {
            return res
                .status(503)
                .json({ error: 'AI is not configured — set OPENROUTER_API_KEY in server/.env' });
        }
...
${propsContext || 'No properties available.'}`;
        ...
        if (err?.code === 'NO_AI_KEY') {
            return res.status(503).json({ error: 'AI unavailable — OPENROUTER_API_KEY not set.' });
        }
